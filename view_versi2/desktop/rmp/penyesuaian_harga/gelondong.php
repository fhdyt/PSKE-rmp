 ...
 <div class="col-lg-12 col-md-12">
   <div class="list-group">
     <div class="list-group-item">
       <div class="row">
         <div class="col-md-8">
           <h3><i class="fa fa-calculator"></i> Gelondong</h3>
           <hr>
         </div>
     ...
     <div class="modal-body">
       <form action="javascript:download();" class="fDataHarga" id="fDataHarga" name="fDataHarga">
         <div class="row">
           ...
           <input autocomplete="off" class="form-control JENIS_MATERIAL" id="JENIS_MATERIAL" name="JENIS_MATERIAL" placeholder="" value="GELONDONG" type="hidden">
...
<div aria-labelledby="myLargeModalLabel" class="modal fade bs-example-modal-lg modalPerbaruiHarga" role="dialog" tabindex="-1">
 <div class="modal-dialog modal-large" role="document">
   <div class="modal-content">
     <div class="modal-header">
       <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
       <h4 class="modal-title" id="myModalLabel">Perbarui Harga Gelondong</h4>
     </div>
     <div class="modal-body">
         ...
         <div class="row">
           <div class="col-md-12">
             <div class="form-group">
               ...
               <p class="help-block"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> <i>Anda akan mengganti seluruh penyesuaian harga supplier untuk kelapa Gelondong.</i></p>
             </div>
           </div>
         </div>
       </form>
...
function supplier_list(curPage)
{
 ...
 $.ajax({
   ...
   url: refseeAPI,
   dataType: 'json',
   data: 'ref=supplier_list_kb&batas=' + $('input#REC_PER_HALAMAN').val() + '&material_id=12&jenis_material=GELONDONG&halaman=' + curPage + '&keyword=' + $("input#keyword").val() + '&' + filter,
     ...
     if (data.respon.pesan == "sukses") {
       ...
       for (i = 0; i < data.result.length; i++)
       {
         ...
         if (data.result[i].STATUS_QUALITED == 'EMPTY'){
           ...
           var harga_patokan_c = "<input autocomplete='off' class='form-control HARGA_PATOKAN_C_" + data.result[i].RMP_MASTER_PERSONAL_ID + "' id='HARGA_PATOKAN_C_" + data.result[i].RMP_MASTER_PERSONAL_ID + "' name='HARGA_PATOKAN_C_" + data.result[i].RMP_MASTER_PERSONAL_ID + "' placeholder='' type='text'><p class='help-block" + data.result[i].RMP_MASTER_PERSONAL_ID + " text-danger'></p>"
           ...
           var harga_patokan_c = data.result[i].HARGA_C
...
$("tbody#zone_data").on('click', '.simpan_harga', function(){
 ...
 var harga_patokan_c = "HARGA_PATOKAN_C=" + $('.HARGA_PATOKAN_C_'+id+'').val() + ""
 ...
 var material = "MATERIAL=12"
 var jenis_material = "JENIS_MATERIAL=GELONDONG"
 var form = "" + id_supplier + "&" + harga_patokan_a + "&" + harga_patokan_b + "&" + harga_patokan_c + "&" + berlaku + "&" + berakhir + "&" + material + "&" + jenis_material + ""
 console.log(form)
 $.ajax({
   type: 'POST',
   url: refseeAPI,
   ...
   success: function(data) {
     if (data.respon.pesan == "sukses") {
       ...
       for (i = 0; i < data.result.length; i++) {
         ...
         "<td>" + data.result[i].RMP_PENYESUAIAN_HARGA_KB_C +  "</td>" +
