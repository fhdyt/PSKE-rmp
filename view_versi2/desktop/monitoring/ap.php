...
.tdTglFixed{ width:90px !important;display: inline-block !important;}
			...
			<div class="list-group-item">
				<div class="row">
					<div class="col-md-12" >
						...
						<!--END FORM-->
						<div class = "outer">
						<table class="table table-hover tabel-utama" style="border: 1px solid #A09D9D;">
							<thead>
							<tr style="border: 1px solid #A09D9D;">
								<th class="text-center">Supplier</th>
								<th class="text-center">No Invoice</th>
								<th class="text-center">Tgl Invoice</th>
								<th class="text-center">Nilai Invoice<br>(Faktur+PPh)</th>
								<th class="text-center">Potongan<br>PPh</th>
								<th class="text-center">Nilai Invoice</th>
								<th class="text-center">No AP</th>
								<th class="text-center">Tgl AP</th>
...
$('button#btn-reload').on('click',function(){
    $("tbody#zone_data").html('<tr><td colspan="13">Loading...!</td></tr>');
    $("tfoot#zone_data_foot").html('');
});
...
function tampil(curPage){
	...
	$.ajax({
		...
		async: false,
		dataType:'json',
		data:'aplikasi=<?php echo $d0;?>&ref=monitoring_ap&'+$("form.fData").serialize(),
		success:function(data)
		{
			...
			if(data.respon.pesan=="sukses")
			{
				...
				if(JENIS_LAPORAN=="Harian" || JENIS_LAPORAN=="Mingguan" || JENIS_LAPORAN=="Pengaturan" || JENIS_LAPORAN=="Bulanan")
				{
					...
					{
						//
						var JumlahFAKTUR = data.result[result].FAKTUR.length;						
						if(JumlahFAKTUR>0)
						{
							var tdFakturKosong="";
						}else{
						...
						//
						var rowspan = 0;
						var detailLength = data.result[result].FAKTUR.length;
						rowspan += detailLength;

						//kebutuhan rowspan tambahan ketika AP ada pada faktur yang ada
						var persiapanRowspanAPTambahan={};
						var jlhAP={};
						var jlhAPbaru=0;
						for (var i = 0; i < detailLength; i++) 
						{
							rowspan += data.result[result].FAKTUR[i].AP.length;
							jlhAP[i]=data.result[result].FAKTUR[i].AP.length;
							
								jlhAPbaru=jlhAPbaru+1;
								persiapanRowspanAPTambahan[data.result[result].RMP_REKENING_RELASI]=jlhAPbaru;
							
							
						}
						var rowspanAPTambahan=persiapanRowspanAPTambahan[data.result[result].RMP_REKENING_RELASI];
						if(rowspanAPTambahan==undefined)
						{
							var rowspanAPTambahanXY=0;
						}else{
							var rowspanAPTambahanXY=rowspanAPTambahan;
						}
						//kebutuhan 1 rowspan tambahan ketika faktur ada
						var JumlahRowFaktur=data.result[result].FAKTUR.length;
						if(JumlahRowFaktur>0)
						{
						...
						tableContent += "<tr class='trBaris'><td class='trBaris' rowspan=" + parseInt(1 + rowspanAPTambahanXY + rowspanTetapFakturX + rowspan) + ">" + data.result[result].RMP_REKENING_RELASI + "<br>"+ data.result[result].RMP_MASTER_PERSONAL_NAMA +"</td>"+tdFakturKosong+"</tr>";
						var APLength = 0;
						for (var i = 0; i < detailLength; i++) 
						{
							var JumlahAP = data.result[result].FAKTUR[i].AP.length;
							if(JumlahAP>0)
							{
								var tdAPKosong="";
							}else{
							...
							APLength = data.result[result].FAKTUR[i].AP.length;
							tableContent += "<tr class='trBaris'>"+
												"<td  class='trBaris text-left' rowspan=" + parseInt(1 + APLength) + ">"+
													data.result[result].FAKTUR[i].NoFaktur +
												"</td>"+
												"<td  class='trBaris text-left' rowspan=" + parseInt(1 + APLength) + ">"+
													"<span class='tdTglFixed'>"+data.result[result].FAKTUR[i].TglInvoices +"</span>"+
												"</td>"+
												"<td  class='trBaris text-right' rowspan=" + parseInt(1 + APLength) + ">"+
												formatNumber(data.result[result].FAKTUR[i].NilaiFakturs) +
												"</td>"+
												"<td  class='trBaris text-right' rowspan=" + parseInt(1 + APLength) + ">"+
												formatNumber(data.result[result].FAKTUR[i].debitnotes) +
												"</td>"+
												"<td  class='trBaris text-right' rowspan=" + parseInt(1 + APLength) + ">"+
												formatNumber(data.result[result].FAKTUR[i].NilaiInvoices) +
												"</td>"+
												tdAPKosong+"</tr>";
												...
												{
													tableContent += "<tr class='trBaris'>"+
															"<td class='trBaris text-left'>" + data.result[result].FAKTUR[i].AP[j].NomorAP + "</td>"+
															"<td class='trBaris text-left'><span class='tdTglFixed'>" + data.result[result].FAKTUR[i].AP[j].Tanggals+ "</span></td>"+
															"<td class='trBaris text-right'>" + formatNumber(data.result[result].FAKTUR[i].AP[j].IDREquivalents) + "</td>"+
															"<td class='trBaris text-left'><span class='tdTglFixed'>" + data.result[result].FAKTUR[i].AP[j].TglBayars + "</span></td>"+
															"<td class='trBaris text-left'>" + data.result[result].FAKTUR[i].AP[j].ReffNo + "</td>"+
															"<td class='trBaris text-right'>" + formatNumber(data.result[result].FAKTUR[i].AP[j].IDREquivalentCBs) + "</td>"+
															"<td class='trBaris text-right'>" + formatNumber(data.result[result].FAKTUR[i].AP[j].HutangAP) + "</td>"+
														"</tr>";
												}
								
								//baris summary masing-masing faktur
								/*
								if(JumlahAP>0)
								{
									tableContent += "<tr class='trSumAP'>"+
															"<td class='trSumAP text-right' colspan='2'>Total Per Invoice</td>"+
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].NilaiFakturs) +"</td>"+
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].debitnotes) +"</td>"+
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].NilaiInvoices) +"</td>"+
															"<td class='trSumAP text-right'>Jlh AP (<b>"+data.result[result].FAKTUR[i].JumlahAPFaktur+"</b>)</td>"+
															"<td class='trSumAP text-right'></td>"+
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].NilaiBayarAP) +"</td>"+
															"<td class='trSumAP text-right' colspan='2'></td>"+
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].NilaiBayarFaktur) +"</td>"+
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].SisaHutangFaktur) +"</td>"+
														"</tr>";
								}else{}
								*/
									tableContent += "<tr class='trSumAP'>"+
															...
															"<td class='trSumAP text-right trBold'>"+formatNumber(data.result[result].FAKTUR[i].SisaHutangFaktur) +"</td>"+
														"</tr>";
								//end sum masing-masing faktur
								
							
						}
						//baris summary seluruh faktur
						if(JumlahFAKTUR>0)
						{
							tableContent += "<tr class='trSumFaktur'>"+
													"<td class='trSumFaktur text-right' colspan='2'>Total per Suplier<br>"+
														"<span class='trInfo'>#"+data.result[result].RMP_REKENING_RELASI+"<br>"+
														data.result[result].RMP_MASTER_PERSONAL_NAMA+"</span>"+
														"<br>Jumlah Invoice (<b>"+data.result[result].JLH_FAKTUR+"</b>)"+
													"</td>"+
													"<td class='trSumFaktur text-right trBold'>"+formatNumber(data.result[result].TotalNilaiFaktur)+"</td>"+
													"<td class='trSumFaktur text-right trBold'>"+formatNumber(data.result[result].Totaldebitnote)+"</td>"+
													"<td class='trSumFaktur text-right trBold'>"+formatNumber(data.result[result].TotalNilaiInvoice)+"</td>"+
													"<td class='trSumFaktur text-right' colspan='2'></td>"+
													"<td class='trSumFaktur text-right trBold'>"+formatNumber(data.result[result].TotalNilaiBayarAP)+"</td>"+
													"<td class='trSumFaktur text-right' colspan='2'></td>"+
													"<td class='trSumFaktur text-right trBold'>"+formatNumber(data.result[result].TotalNilaiBayarFaktur)+"</td>"+
													"<td class='trSumFaktur text-right trBold'>"+formatNumber(data.result[result].TotalSisaHutangFaktur)+"</td>"+
												"</tr>";
						}else{}
						//end sum seluruh faktur
					}
					//sum keseluruhan suplier
					var tableFoot = "<tr class='trSumFaktur'>"+
													"<td class='text-right' colspan='3'><span class='trBold'>Total Keseluruhan</span><br>"+
														"Jumlah Invoice (<b>"+data.resultTotal.TOTAL_KESELURUHAN_JLH_FAKTUR+"</b>)"+
													"</td>"+
													"<td class='text-right trBold'>"+formatNumber(data.resultTotal.TotalKeseluruhanNilaiFaktur)+"</td>"+
													"<td class='text-right trBold'>"+formatNumber(data.resultTotal.TotalKeseluruhandebitnote)+"</td>"+
													"<td class='text-right trBold'>"+formatNumber(data.resultTotal.TotalKeseluruhanNilaiInvoice)+"</td>"+
													"<td class='text-right' colspan='2'></td>"+
													"<td class='text-right trBold'>"+formatNumber(data.resultTotal.TotalKeseluruhanNilaiBayarAP)+"</td>"+
													"<td class='text-right' colspan='2'></td>"+
													"<td class='text-right trBold'>"+formatNumber(data.resultTotal.TotalKeseluruhanNilaiBayarFaktur)+"</td>"+
													"<td class='text-right trBold'>"+formatNumber(data.resultTotal.TotalKeseluruhanSisaHutangFaktur)+"</td>"+
												"</tr>";
				...
				}else
				{
              	...
              	$("tfoot#zone_data_foot").html(tableFoot);
